...
*/
const temperatureInF = (temperature, unit) => {
  if (unit === 'F') {
...
*/
const isItAnApple = (arr) => {
  let result = [];
  for (let i = 0; i < arr.length; i++) {
    result.push(arr[i] === "apple");
  }
  return result;
}
...
*/
const getOlderCars = (arr, maxYear) => {
  const result = [];
  for (let i = 0; i < arr.length; i++) {
